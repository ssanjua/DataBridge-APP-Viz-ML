...
st.set_page_config(page_title = 'PROYECT - DataBrick APP', initial_sidebar_state='expanded')
...
df_count = pd.read_csv('./data/df_reviews_anio_small.csv')
conteo_resenas = df_count.groupby('year')['stars'].count()
st.line_chart(conteo_resenas)

df_map = pd.read_csv('./data/df_bussiness_small.csv')
...
st.map(data=df_map, zoom=3, use_container_width=True)
...
st.write('El flujo de trabajo completo involucra la configuración de la API de Drive, la extracción de datos desde Google Drive, la carga de datos en Google Cloud Store, el procesamiento de datos con Dataflow y la carga de datos en BigQuery. Esto nos permite tener datos limpios y ordenados en nuestro data warehouse de BigQuery obteniendo así información valiosa y confiable para nuestro proyecto.')
...
with header_col:
    st.header('Te gustaría saber más? :robot_face:')

# Mostrar el campo de entrada en la segunda columna
with input_col:
    mood = st.text_input(' :smiley:  :confused: :sob: :grin:')
    st.markdown('#### A la AI :robot_face: le parece vamos rumbo a algo ' + get_sentiment(mood))
